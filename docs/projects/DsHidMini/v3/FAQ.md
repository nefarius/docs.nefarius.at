...
ControlApp is a companion tool, not part of the driver MSI. Download it from the [build server](https://buildbot.nefarius.at/builds/DsHidMini/latest/bin/ControlApp.exe). It needs the [.NET Desktop Runtime 10](https://dotnet.microsoft.com/en-us/download/dotnet/10.0). You do not have to keep it open for the driver to work.
