...
3. Remove the class filters that were added during installation:
    ...
    ```powershell
    # HIDClass
    nefconc --remove-class-filter --position upper --service-name HidHide --class-guid 745a17a0-74d3-11d0-b6fe-00a0c90f57da
    # GUID_DEVCLASS_XNACOMPOSITE
    nefconc --remove-class-filter --position upper --service-name HidHide --class-guid d61ca365-5af4-4486-998b-9db4734c6ca3
    # GUID_DEVCLASS_XBOXCOMPOSITE
    nefconc --remove-class-filter --position upper --service-name HidHide --class-guid 05f5cfe2-4733-4950-a6bb-07aad01a3a84
    ```

4. Delete the configuration client files you copied earlier.
5. Reboot Windows to remove the filter from device stacks.
